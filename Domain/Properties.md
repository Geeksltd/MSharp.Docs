...
| .Default(string value)                   | DateTime("Date created").Default("LocalTime.Now").Mandatory();                         | Default attribute will have no effect on the database column definition, but will change your class constructor in the generated C# file and initialize int with default value. |
...
| .ColumnName(string value)                | String("Last name").ColumnName("Surname");                                             | Column name attribute will have no effect on the generated C# file. This will only change the column name in the database, this change is handled by the data access layer and does not change your entity file. |
| .DatabaseIndex(bool? value = true)       | Associate\<Employee\>("Status").Default("Pending").DatabaseIndex();                    | Database index has no effect on the generated C# file. This attribute will create an index in the database for this column. |
| .DefaultFormatString(string value)       | String("Name").DefaultFormatString("Employee {0}");                                    | Default format string attribute will have no effect on the database column definition or the generated C# class. It allows you to display a custom expression in Views. Example: The format "Employee {0}" will display "Employee Smith" if the value of the property is "Smith". |
| .Documentation(string value)             | String("Email").Documentation("Email address of the employee, also used as username"); | Documentation attribute will have no effect on the database column definition or the generated C# class. It adds a piece of information to your property XML documentation, for example the full name of the property, how to use it, how it works, ... |
...
| .HelpText(string value)                  | Associate\<Employee\>("Status").HelpText("The current status for this user");          | Help text attribute will have no effect on the database column definition or the generated C# class. This attribute generates a help icon next to the control in CSHTML forms. |
...
| .Name(string value)                      | String("Last name").Name("Suranme");                                                   | By setting a custom name, M# will change the name of your property and the name of your SQL column. This change will not have any impact on the UI because the Name attribute is independent of the Title attribute. |
...
Calculated properties are created for read-only purposes and returns data based on business requirements. M# defines these types of properties in “Entity Class” of the “Model Project” and marks them with “Calculated” attribute. No SQL table Column is created for such properties. A calculated property is usually used to display some information on UI or in decision making. A good example of this could be to have a “Full Name” property on “Member” entity, as shown below.
...
#### Database index
Database index has no effect on the generated C# file. This attribute will create an index in the database for this column. M# will often suggest you to specify an index on foreign key columns to improve the performance of SQL queries.
